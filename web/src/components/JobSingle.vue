<template>
  <div>
    ...
    <el-dialog
      ...
      v-if="userInfo"
    >
      <el-form :model="resumeForm" :rules="resumeFormRules" ref="resumeForm" label-position="top">
        <el-form-item label prop="resume">
          <el-radio-group v-model="resumeForm.resume_id">
            <el-radio v-for="(item,index) in userInfo.resumes" :key="index" :label="item.id">
              {{item.resume_name}}
              （
              <router-link :to="{name:'ResumeSingle',params:{resumeId:item.id}}">简历详情</router-link>）
            </el-radio>
          </el-radio-group>
        </el-form-item>
      </el-form>
              ...
              <div class="sidebar-widget" v-if="this.store.state.userRole==='user'">
                <div class="inner">
                  <div class="row m-n2">
                    ...
                    <div class="col-xl-auto col-lg-12 col-sm-auto col-12 p-2">
                      <el-button v-if="!isApplyForJob" @click="showResumes" type="success" plain>
              ...
              <div class="sidebar-widget">
                <div class="inner">
                  ...
                  <ul class="job-overview list-unstyled">
                    ...
                    <li>
                      <strong>成立时间：</strong>
                      {{jobInfo.company.company_create}}
                    </li>
                    <li>
                      <strong>公司规模：</strong>
                      {{jobInfo.company.company_size}}
                    </li>
                    <li>
                      <strong>公司地址：</strong>
                      {{jobInfo.company.company_address}}
                    </li>
                    <li>
                      <strong>公司网站：</strong>
                      <a :href="jobInfo.company.company_site" target="_blank">{{jobInfo.company.company_site}}</a>
                    </li>
                    <li>
                      <strong>联系人：</strong>
                      {{jobInfo.company.company_contacts}}
                    </li>
                    <li>
                      <strong>联系电话：</strong>
                      {{jobInfo.company.company_phone}}
                    </li>
                  </ul>
                </div>
              </div>
...
export default {
    ...
    data() {
        return {
            ...
            resumeFormRules: {
                resume_id: [{ required: true, message: '请输选择简历', trigger: 'blur' }]
            },
            jobInfo: null,
            userInfo: null,
            isLikeJob: false,
